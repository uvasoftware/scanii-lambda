...
let scaniiAPISecret = undefined;

const getScaniiAPISecrets = function() {
  const headers = {"X-Aws-Parameters-Secrets-Token": process.env.AWS_SESSION_TOKEN}
  const scaniiAPISecretName = process.env.SCANII_API_SECRETS_NAME;
  
  if (scaniiAPISecret) {
    console.log('using cached scanii secret');
    return Promise.resolve(scaniiAPISecret);
  }

  return fetch(`http://localhost:2773/secretsmanager/get?secretId=${scaniiAPISecretName}`, {
    method: 'GET',
    headers: headers,
  })
    ...
    .then(function (data) {
      scaniiAPISecret = data.SecretString;
      return scaniiAPISecret;
    })
};
